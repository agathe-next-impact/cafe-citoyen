...
export function AnimatedNav({
  ...
  return (
    <div className={cn("w-full bg-white", className)}>
      <nav
        className={cn(
          "fixed top-0 left-0 right-0 z-100 transition-all duration-300 max-h-20 backdrop-blur-lg border-b border-blue-100",
          isScrolled ? "bg-[var(--background)/0.95]" : `bg-[${baseColor}/0.8]`,
          isExpanded ? "border-b border-blue-100" : " border-b border-blue-100",
        )}
      >
        <div className="container mx-auto max-w-full overflow-x-hidden">
          <div className="flex items-start justify-between pt-4 px-4">
            ...
            <button
              onClick={toggleMenu}
              className={cn(
                "tracking-wider hover:shadow-md bg-blue-600 hover:bg-blue-600/90 rounded-full mt-2 ml-2 px-4 py-2 shadow-sm gap-1.5 relative z-110 cursor-pointer transition-all",
                isExpanded && "gap-0",
              )}
        ...
        <div
          className={cn(
            "overflow-hidden transition-all duration-500 ease-out -mt-14",
          )}
        >
